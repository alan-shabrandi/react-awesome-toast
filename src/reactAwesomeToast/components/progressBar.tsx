...
const ProgressBar = ({type, positionX, setShowToast, autoClose, theme}: ProgressBarProps) => {
    const [width, setWidth] = useState<number>(100);

    const intervalRef = useRef<any>(null)

    useEffect(() => {
        if(autoClose) intervalRef.current = setInterval(() => setWidth((prev) => prev - (1000 / autoClose)), 10)
